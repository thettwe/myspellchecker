...
AUXILIARY_VERBS: frozenset[str] = frozenset(
    {
        ...
        "ခဲ့",  # Past tense (လုပ်ခဲ့တယ် - did, f=1.6M)
        "ဖူး",  # Experiential (စားဖူးတယ် - have eaten before, f=78K)
        ...
        "ဝင်",  # Enter (ဝင်လာတယ် - come in, f=194K)
        "ထွက်",  # Exit (ထွက်သွားတယ် - go out, f=215K)
        "ရောက်",  # Arrive (ရောက်ရှိတယ် - arrive at, f=185K)
        # Manner/result markers
        "မိ",  # Accidentally (ကျမိတယ် - fell accidentally, f=138K)
        "ပစ်",  # Discard/away (ပစ်ချတယ် - throw down, f=50K)
        "ခံ",  # Passive/endure (ဒဏ်ခံတယ် - suffer, f=228K)
        "ယူ",  # Take/acquire (သယ်ယူတယ် - carry, f=160K)
        "ထည့်",  # Put in (ထည့်သွင်းတယ် - insert)
        "ထုတ်",  # Out (ထုတ်ပြတယ် - bring out, f=140K)
        "လွန်",  # Exceed (များလွန်းတယ် - too many, f=48K)
        "ပြတ်",  # Sudden/complete (ပြတ်သွားတယ် - snapped, f=14K)
        # Benefactive/causative
        "ပေး",  # For someone (ကူညီပေးတယ် - helped for someone)
        "စေ",  # Causative (လုပ်စေတယ် - made do)
        "ခိုင်း",  # Order/delegate (ခိုင်းစေတယ် - order to do, f=25K)
        # Modal auxiliaries
        ...
        "ရဲ",  # Dare (ပြောရဲတယ် - dare to say, f=58K)
        "အပ်",  # Obligation (လုပ်အပ်တယ် - ought to do, f=37K)
        "ဖို့",  # Purpose (လုပ်ဖို့ - in order to do, f=577K)
        # Attempt/effort
        "ကြည့်",  # Try (လုပ်ကြည့်တယ် - try doing)
        "ကြ",  # Plural action (သွားကြတယ် - they go)
        # Repetition/habitual
        "ပြန်",  # Again (ပြောပြန်တယ် - say again)
        "လေ့ရှိ",  # Habitually (သွားလေ့ရှိတယ် - usually go)
    }
)
